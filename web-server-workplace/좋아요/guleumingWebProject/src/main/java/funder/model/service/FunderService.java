...
public class FunderService {
	//1. 글 목록 조회
	public FunderPageData selectFunderList(int reqPage, String category, int memberNo) {
		...
		if(category.equals("전체")) {			
			mblist = dao.selectFunderList(conn,start,end,memberNo);
			totalCount = dao.selectTotalCount(conn);
		}else {
			mblist = dao.selectFunderList(conn,start,end,category,memberNo);
	...
	//8. 펀더 상세보기
	public MakerBoard selectOneFunderView(int writerNo, int memberNo) {
		Connection conn = JDBCTemplate.getConnection();
		MakerBoard mb = new FunderDao().selectOneFunderLikeView(conn,writerNo,memberNo);
		JDBCTemplate.close(conn);
		return mb;
	}	
	//09. 게시물 검색
	public FunderPageData searchFunder(int reqPage, String keyword, int memberNo) {
		Connection conn = JDBCTemplate.getConnection();
		int numPerPage = 5;
		int end = reqPage * numPerPage;
		int start = end - numPerPage + 1;
		FunderDao dao = new FunderDao();
		ArrayList<MakerBoard> mblist = dao.selectSearchFunder(conn, start, end, keyword,memberNo);
		...
		if(pageNo != 1) {
			pageNavi += "<li class='page_item'>";
			pageNavi += "<a class='page_link' href='/searchFunder?reqPage="+(pageNo-1)+"&keyword="+keyword+"'>";
			pageNavi += "&lt;</a></li>";
		}
		for(int i = 0; i < pageNaviSize;i++) {
			if(pageNo == reqPage) {
				pageNavi += "<li class='page_item active'>";
				pageNavi += "<a class='page_link' href='/searchFunder?reqPage="+pageNo+"&keyword="+keyword+"'>";
				pageNavi += pageNo+"</a></li>";
			}else {
				pageNavi += "<li class='page_item'>";
				pageNavi += "<a class='page_link' href='/searchFunder?reqPage="+pageNo+"&keyword="+keyword+"'>";
	...
	//10. 정보 수정
	public int updateFunder(MakerBoard mb) {
		Connection conn = JDBCTemplate.getConnection();
		int result = new FunderDao().updateFunder(conn,mb);
		if(result > 0) {
			JDBCTemplate.commit(conn);
		}else {
			JDBCTemplate.rollback(conn);
	...
	//11. 프로젝트 조회
	public ArrayList<FundingListRecent> selectProject(int start, int writerNo) {
		Connection conn = JDBCTemplate.getConnection();
		ArrayList<FundingListRecent> pro = new FunderDao().selectProject(conn,start,writerNo);
		for(FundingListRecent fl : pro) {
			LocalDate now = LocalDate.now();
			String endDate = fl.getEndDate();
			LocalDate date = LocalDate.parse(endDate, DateTimeFormatter.ISO_DATE);
			long period = ChronoUnit.DAYS.between(now, date);
			fl.setPeriod(period);
			int totalPrice = fl.getTotal()*fl.getRewardPrice();
			fl.setTotalPrice(totalPrice);
			int percent = fl.getTotalPrice()/fl.getTargetPrice()*100;
			fl.setPercent(percent);
		}
		JDBCTemplate.close(conn);
		return pro;
	}
	//12. list 조회 시 조회
	public MakerBoard selectOneFunderView(int memberNo) {
		Connection conn = JDBCTemplate.getConnection();
		MakerBoard mb = new FunderDao().selectOneFunderView(conn,memberNo);
		JDBCTemplate.close(conn);
		return mb;
	}
}
