...
public class LoginDao {
	...
	public void BusinessMember(Connection conn, int busiNo, Member member) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		String query = "select * from business where business_no=?";
		try {
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, busiNo);
			rset = pstmt.executeQuery();
			if(rset.next()) {
				member.setBusinessCode(rset.getString("business_code"));
				member.setBusinessName(rset.getString("business_name"));
				member.setBusinessNo(rset.getInt("business_no"));
				member.setManagerName(rset.getString("manager_name"));
