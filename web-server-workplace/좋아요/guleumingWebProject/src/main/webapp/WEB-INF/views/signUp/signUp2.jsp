...
<body>
	...
	<h1 class="login_title">JOIN</h1>
		<div class="join_tab">
			<span id="user"><a href="/join">개인회원</a></span> <span id="maker" style="background: #00B9CE;color: #fff;">법인회원</span>
		</div>
		<div id="join" class="join_form">
			<form action="/joinFrm2" method="post">
				...
				<div class="login_box">
					...
					<input type="text" id="busiName" name="busiName" class="input_01">
				</div>
				<div class="login_box">
					<label for="busiCode">사업자 등록번호</label>
					 <input type="text" id="busiCode"	name="busiCode" class="input_01">
				</div>
				<div class="login_box2">
					<label for="managerName">담당자명</label>
					 <input type="text"	id="managerName" name="managerName" class="input_01">
				 </div>
				<input type="hidden" name ="joinLevel" id="joinLevel" value="3">
				<input type="submit" value="회원가입 " name="joinSub" class="btn btn_100 login_btn margin_b">
			</form>
		</div>
	</div>
	<script>
	$(function() {
			...
			var reName = true;
			var rePhone = true;
			var nameVal = $("input[name=busiName]").val();
			var codeVal = $("input[name=busiCode]").val();
			var managerVal =  $("input[name=managerName]").val();
			...
			$("input[type=submit]").click(function() {
				 if(reId != true || rePw != true || rePwRe != true || reName != true || rePhone != true || nameVal==null || codeVal==null || managerVal==null) {                            
		               alert("형식이 맞지 않습니다");   
		               return false;          
		         }
		});
