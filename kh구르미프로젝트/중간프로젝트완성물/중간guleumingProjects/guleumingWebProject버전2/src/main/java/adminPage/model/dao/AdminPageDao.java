...
public class AdminPageDao {
	...
	public ArrayList<AdminPageFundingRewardPayment> selectAdminPageFundingList(Connection conn, int start, int end) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		ArrayList<AdminPageFundingRewardPayment> list = new ArrayList<AdminPageFundingRewardPayment>();
		...
		String query = "select * from (select t.* from (select rownum as rnum, (select sum(quantity) from payment_info pi where pi.project_no = pbi.project_no) as total, r.reward_price, pbi.* from project_basic_info pbi join reward r on pbi.project_no = r.reward_no order by pbi.project_no desc)t)t2 where t2.rnum between ? and ?";
		try {
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, start);
			pstmt.setInt(2, end);
			rset = pstmt.executeQuery();
			while(rset.next()) {
				AdminPageFundingRewardPayment pbi = new AdminPageFundingRewardPayment();
				...
				pbi.setRewardPrice(rset.getInt("reward_price"));
				pbi.setTotal(rset.getInt("total"));
				list.add(pbi);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
