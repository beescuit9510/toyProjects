				...
				<c:forEach items="${fcList }" var="fc">
					<li>
						<a class= "categorysearch" category="${fc.fundingCategory }">
							<c:choose>
								<c:when test="${fc.fundingCategory == '가전'}">
	                        		<img src="/img/Parksowon/category01.jpg">
								</c:when>
								<c:when test="${fc.fundingCategory == '도서'}">
	                        		<img src="/img/Parksowon/category02.jpg">
								</c:when>
								<c:when test="${fc.fundingCategory == '리빙'}">
	                        		<img src="/img/Parksowon/category03.jpg">
								</c:when>
								<c:when test="${fc.fundingCategory == '반려동물'}">
	                        		<img src="/img/Parksowon/category04.jpg">
								</c:when>
								<c:when test="${fc.fundingCategory == '악세서리'}">
	                        		<img src="/img/Parksowon/category05.jpg">
								</c:when>
								<c:when test="${fc.fundingCategory == '여행레저'}">
	                        		<img src="/img/Parksowon/category06.jpg">
								</c:when>
								<c:when test="${fc.fundingCategory == '푸드'}">
	                        		<img src="/img/Parksowon/category07.jpg">
								</c:when>
							</c:choose>
	                        <span>${fc.fundingCategory}</span>
	                    </a>
					</li>
				</c:forEach>
			</ul>
